...
namespace JacRed.Controllers.CRON
{
    ...
    public class LostfilmController : BaseController
    {
        ...
        async Task<bool> parsePage(int page)
        {
            ...
            foreach (string row in tParse.ReplaceBadNames(html).Split("class=\"hor-breaker dashed\"").Skip(1))
            {
                try
                {
                    ...
                    if (tParse.TryGetValue(url, out TorrentDetails _t) && !string.IsNullOrWhiteSpace(_t.magnet))
                        continue;
                    ...
                    tParse.AddOrUpdate(new TorrentDetails()
                    {
                        ...
                        url = url,
                        title = $"{name} / {originalname} / {sinfo} [{relased}, {mag.quality}]",
                        sid = 1,
        ...
        async public Task<string> parseMagnet()
        {
            ...
            try
            {
                foreach (var torrent in tParse.db.Where(i => i.Value.trackerName == "lostfilm" && string.IsNullOrWhiteSpace(i.Value.magnet)))
                {
                    var mag = await getMagnet(torrent.Value.url);
                    if (!string.IsNullOrWhiteSpace(mag.magnet))
                    {
                        torrent.Value.title = torrent.Value.title.Replace(", ]", $", {mag.quality}]");
